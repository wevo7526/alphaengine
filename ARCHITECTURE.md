...
## Appendix: Image-generation prompt for the updated architecture diagram

Use this prompt verbatim with an image-generation tool (Midjourney, DALL-E 3, Imagen, Nano Banana). It's calibrated to extend the existing architecture image (Alpha Engine logo top-left, Anthropic Claude tag in the agent pipeline, dark navy/teal/purple palette, white sans-serif labels) so the new render is a true update, not a parallel design.

```
A wide-format technical architecture diagram for an AI-powered hedge fund operating system called "Alpha Engine," dark navy background (#0B1220), teal/cyan accents (#3DD9D6) for data flow arrows, magenta/purple accents (#A06AFF) for the quantitative engine, white sans-serif labels (Inter or SF Pro). Layout in three vertical bands.

TOP BAND — USER INTERFACE: six panels in a single row, each a small rectangle with a thin border. Labels: "Home Dashboard" (macro charts, regime badge, morning report, live P&L mini-card), "Analysis" (freeform query, real-time agent trace, intelligence memo with grounding badge), "Portfolio" (HITL trade journal, positions, scorecard, attribution, factors), "Risk" (VaR with Cornish-Fisher + bootstrap CI, CVaR, sector exposure, correlation heatmap, stress panel for GFC/COVID/2022, regime probabilities), "Settings" (live system status, data source health, env-driven risk thresholds), "Auth" (Clerk-gated, JWKS verified). Small "SSE streaming" arrow flowing down from the UI band into the agent pipeline.

MIDDLE BAND — AI AGENT PIPELINE: a single rounded container labeled "AI Agent Pipeline" with an "Anthropic Claude" tag at the top right. Inside, five connected nodes left-to-right with horizontal arrows: "Query Interpreter" (parses intent, plan_confidence score), "Research Analyst" (22 tools: macro, fundamentals, news with descriptions, SEC filings + extracted sections, options analytics with vol term structure, earnings calendar, analyst consensus, peer comparison, short interest, FinBERT-style sentiment), "Risk Manager" (5 tools, hard-capped: realized correlation, factor loadings, market breadth, macro snapshot, VIX history; track-record-aware), "Portfolio Strategist" (sees existing book + sector caps + scorecard calibration; emits 5 trade ideas with entry/stop/target and 5 cross-asset hedges across rates/credit/FX/sector pairs/vol), "CIO Synthesizer" (writes 800-word memo with prior-memo continuity via RAG). Below the five nodes: small subtitle "LangGraph orchestration + JSON corrective re-prompt + tool-grounding tripwire." A small parallel block "Decision Gate" sitting under the CIO node with arrows showing GO/WATCH/NO-GO output and a "track-record adjustment" sub-label.

LEFT EDGE — DATA SOURCES (vertical column of stacked rounded green rectangles): FRED (13 macro indicators, risk-free rate from DGS3MO), Yahoo Finance (OHLCV, fundamentals, options chains, total-return-adjusted history), NewsAPI (100/day, with descriptions), Finnhub (sentiment, company news with summaries), SEC EDGAR (8-K, 10-K, 10-Q, MD&A and Risk Factors sections, insider trades, 13F), Alpha Vantage (RSI, MACD, Bollinger), Firecrawl (web validation, optional). Each box has a small fire/database icon. Arrows flow from these into the agent pipeline middle band.

RIGHT EDGE — QUANTITATIVE ENGINE (vertical column of stacked rounded purple rectangles, header "Quantitative Engine — pure-math numpy/scipy, zero LLM"): "Risk Management" (EWMA covariance + Ledoit-Wolf shrinkage, parametric/Cornish-Fisher/bootstrap VaR, sector limits with hardcoded GICS fallback, drawdown circuit breaker on real book P&L, marginal VaR, liquidity check via ADV/days-to-liquidate/spread), "Performance" (Sharpe, Sortino, Calmar, alpha, beta, rolling Sharpe), "Regime Detection" (HMM with hysteresis + 5-day flip confirmation; risk_on/risk_off/transition; serves as a position-size multiplier), "Stress Testing" (historical scenarios: GFC 2008, COVID 2020, 2022 rate shock, dot-com; parametric shocks: VIX/credit/oil; risk-off cocktail), "Backtesting" (walk-forward, no look-ahead, total-return adjusted, sqrt market-impact slippage), "Factor Analysis" (single-factor + FF5 + Momentum via ETF proxies, alpha p-value with significance flag, attribution), "Signal Validation" (IC date-aligned, ICIR, hit rate by conviction bucket, alpha decay half-life curve fit), "Portfolio Optimization" (Black-Litterman with corrected confidence mapping, mean-variance with turnover penalty, single-source position cap). Arrows flow from this column INTO the agent pipeline.

BOTTOM BAND — PERSISTENCE: a single horizontal rectangle labeled "Persistence — PostgreSQL on Railway, TIMESTAMPTZ schema." Subtitle bullets: "Intelligence memos, Trade journal entries, Portfolio snapshots / live P&L, Backtest runs and results, Regime states, Macro snapshots, Signal scorecards, Watchlist." Small Postgres elephant icon on the left.

Visual style: clean technical diagram, thin 1px borders, soft glow on the central agent pipeline container, no photorealism, no people, no clutter. Aspect ratio 16:9. Resolution 2560x1440.
```

> **Plain English:** Paste the prompt above into your image tool and you'll get the new architecture diagram — same look as the existing one, but reflecting every layer that has been added since (HITL trading, stress panel, hardened risk gates, FF5 + Momentum, RAG continuity, grounding tripwire, all the rest).
