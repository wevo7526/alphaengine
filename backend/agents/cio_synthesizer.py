...
class CIOSynthesizer:
    ...
    async def synthesize(self, context: dict, callbacks: list | None = None) -> AgentOutput:
        ...
        user_prompt = (
            ...
            f"INLINE CITATIONS in the `analysis` prose:\n"
            f"After every numerical claim or specific factual statement, emit a citation marker in "
            f"the format `[[src:<type>:<id>]]`. The platform replaces these with numbered footnotes "
            f"like `[1]`, `[2]` and builds a citation index. Use the same source_type values as the "
            f"upstream agents:\n"
            f"  - `[[src:market_price:AAPL@yfinance]]` for live prices and fundamentals\n"
            f"  - `[[src:sec_filing:0001067983-25-000412]]` for SEC accession numbers\n"
            f"  - `[[src:fred_series:VIXCLS]]` for macro indicators\n"
            f"  - `[[src:computed:realized_correlation]]` for computed analytics\n"
            f"  - `[[src:news_article:headline_fragment]]` for news\n"
            f"Reference IDs that appear in the trade_ideas[].citations or risk_factors[].citations "
            f"arrays above — those have already been validated against the tool-call history. "
            f"Unresolved markers are stripped silently, so prefer ones you can ground."
        )
