...
class RiskManager(BaseAgent):
    ...
    def build_input_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            f"CITATIONS — required on every risk factor:\n"
            f"For each risk factor, include a `citations` array referencing the tool calls that "
            f"surfaced the risk. Each citation is "
            f"`{{\"source_type\": \"market_price|sec_filing|fred_series|computed|news_article\", "
            f"\"source_id\": \"<canonical id>\"}}`. Examples:\n"
            f"  - {{\"source_type\": \"computed\", \"source_id\": \"realized_correlation\"}}\n"
            f"  - {{\"source_type\": \"fred_series\", \"source_id\": \"BAMLH0A0HYM2\"}}\n"
            f"  - {{\"source_type\": \"sec_filing\", \"source_id\": \"0001234567-26-000001\"}}\n"
            f"Aim for 1-3 citations per risk factor. The platform fills in URLs/labels from "
            f"your tool-call history; you just emit the (type, id) pair."
        )
