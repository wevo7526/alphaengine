...
class PortfolioStrategist(BaseAgent):
    ...
    def build_input_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            f"You may optionally include a `citations` array on each idea — a short list of "
            f"source_type/source_id pairs referencing tool calls you used (e.g. market_price for "
            f"yfinance, sec_filing for accession numbers, fred_series for FRED IDs). Citations are "
            f"OPTIONAL — the platform auto-fills them from your tool-call history when omitted. "
            f"Producing the trade ideas themselves is the top priority."
        )
