...
class PortfolioStrategist(BaseAgent):
    ...
    def build_input_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            f"CITATIONS — required on every trade idea:\n"
            f"For each trade idea, include a `citations` array referencing the tool calls that informed it. "
            f"Each citation is `{{\"source_type\": \"market_price|sec_filing|sec_insider|sec_13f|news_article|fred_series|technical|screen\", "
            f"\"source_id\": \"<the canonical id>\"}}`. Examples:\n"
            f"  - {{\"source_type\": \"market_price\", \"source_id\": \"AAPL@yfinance\"}}\n"
            f"  - {{\"source_type\": \"sec_filing\", \"source_id\": \"0001067983-25-000412\"}}\n"
            f"  - {{\"source_type\": \"fred_series\", \"source_id\": \"VIXCLS\"}}\n"
            f"The platform resolves URLs and labels from your tool-call history — you only emit "
            f"the (type, id) pair. Cite EVERY claim, especially live prices, fundamentals, and "
            f"screen evidence. Aim for 2-5 citations per idea. Empty citations are auto-filled "
            f"from any tool calls that touched the ticker, but explicit attribution is preferred."
        )
