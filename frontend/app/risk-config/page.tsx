...
interface RiskParam {
  ...
  desc: string;
  group: string;
  value: number;       // canonical stored value (decimal for pct fields)
  default: number | null;
...
/**
 * Display a stored value as the user-friendly editable string.
 * Pct fields (stored as 0.05) are shown as 5 (meaning 5%).
 */
function toDisplay(value: number, scale: "pct" | "raw"): string {
  ...
  // raw — strip trailing zeros
  const fixed = value.toFixed(4);
  return parseFloat(fixed).toString();
}

/** Inverse of toDisplay — user edits a number, we get the stored value. */
...
export default function RiskConfigPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((d) => {
        ...
        setParams(list);
        // Seed draft inputs from current values
  ...
  async function saveField(field: string) {
    ...
    if (parsed === null) {
      ...
      // Range check client-side first so we can show error without a roundtrip
  ...
  function resetSingle(field: string) {
    ...
    // Trigger save with empty draft → null payload
  ...
  return (
    <div className="p-8 max-w-[1280px] mx-auto space-y-6">
      ...
      {loading ? (
        <p className="text-[12px] text-text-quaternary font-mono">Loading risk gates…</p>
      ) : (
